...
def dodaj_autora(autor: Autori, korisnik: str = Depends(trenutni_korisnik)):
    kursor.execute("INSERT INTO autori (ime) VALUES (%s)", (autor.ime, ))
    konekcija.commit()
...
def dodaj_knjigu(knjiga: Knjige, autor_id: int, korisnik: str = Depends(trenutni_korisnik)):
    kursor.execute("SELECT * FROM autori WHERE id = %s", (autor_id,))
    ...
    kursor.execute(
        "INSERT INTO knjige (naslov, godina, ocjena, autor_id) VALUES (%s, %s, %s, %s)",
...
def azuriraj_knjigu(id: int, knjiga: Knjige, autor_id: int, korisnik: str = Depends(trenutni_korisnik)):
    kursor.execute("SELECT * FROM knjige WHERE id = %s", (id,))
    ...
    kursor.execute(
        "UPDATE knjige SET naslov = %s, godina = %s, ocjena = %s, autor_id = %s WHERE id = %s",
...
def obrisi_autora(id: int, korisnik: str = Depends(trenutni_korisnik)):
    kursor.execute("SELECT * FROM autori WHERE id = %s", (id,))
    postoji = kursor.fetchone()

    if postoji is None:
        raise HTTPException(status_code=404, detail="Autor sa tim id brojem ne postoji")
        
    kursor.execute("DELETE FROM knjige WHERE autor_id = %s", (id,))
    kursor.execute("DELETE FROM autori WHERE id = %s", (id,))
    konekcija.commit()
...
def obrisi_knjigu(id: int, korisnik: str = Depends(trenutni_korisnik)):
    ...
    kursor.execute(
        "DELETE FROM knjige WHERE id = %s", (id, )
    )
    konekcija.commit()
